...
RUN set -e; \
    if [ "$FRONTEND_VERSION" = "latest" ]; then \
      FRONTEND_VERSION=$(curl -fsSL \
        https://api.github.com/repos/collei-monitor/collei-web/releases/latest \
        | grep -m1 '"tag_name"' | cut -d'"' -f4); \
      if [ -z "$FRONTEND_VERSION" ]; then \
        echo "ERROR: Failed to resolve latest frontend version from GitHub API." >&2; \
        exit 1; \
      fi; \
    fi; \
